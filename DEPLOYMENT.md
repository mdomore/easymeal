...
### Option A: Using SCP (NOT RECOMMENDED - will copy data directory)
⚠️ **Warning:** SCP will copy everything including local database. Use rsync instead.
...
### Option C: Using rsync (recommended - excludes data)

First, install rsync on VPS if not already installed:
```bash
ssh debian@vps-72277d08.vps.ovh.net "apt update && apt install -y rsync"
```
...
rsync -avz --progress \
    ...
    --exclude '*.sqlite' \
    --exclude '*.sqlite3' \
    /Users/michaelmoreliere/Dev/easymeal_reboot/ \
    debian@vps-72277d08.vps.ovh.net:/opt/easymeal/easymeal_reboot/
```

**Important:** Always exclude the `data` directory to prevent overwriting your production database!

### Option D: Using tar (if rsync not available)
```bash
tar --exclude='data' --exclude='.git' --exclude='*.db' -czf - . | \
    ssh debian@vps-72277d08.vps.ovh.net "cd /opt/easymeal/easymeal_reboot && tar -xzf -"
```
...
**⚠️ IMPORTANT:** Always exclude the `data` directory when deploying to prevent overwriting your production database!

The deployment script and rsync commands automatically exclude:
- `data/` directory (contains your SQLite database)
...
Or manually:
```bash
cp data/meals.db backups/meals_$(date +%Y%m%d_%H%M%S).db
```
...
```bash
# List backups
ls -lh /opt/easymeal/backups/

# Restore from backup (replace with actual backup file)
cp /opt/easymeal/backups/meals_YYYYMMDD_HHMMSS.db /opt/easymeal/easymeal_reboot/data/meals.db
